...
void AsyncEventLayer::OnAttach() {
  // Initialize the plotting library context
  ...
  Nodens::AsyncEventBus::Get().Subscribe<PlanetaryScanEvent>(
      [this](PlanetaryScanEvent& e) {
        ...
        std::this_thread::sleep_for(
            std::chrono::milliseconds(sleepDist(generator)));

        // Generate "Scientific Data"
        std::uniform_real_distribution<float> distDist(
            0.1f, 100.0f);  // 0 to 100 Light Years
        std::normal_distribution<float> densityDist(
            0.5f, 0.15f);  // Atmosphere density
...
void AsyncEventLayer::OnDetach() { ImPlot::DestroyContext(); }

void AsyncEventLayer::OnUpdate(Nodens::TimeStep ts) { m_TimePassed += ts; }
...
void AsyncEventLayer::AddResult(const PlanetaryScanEvent& e) {
  ...
  if (e.m_CalculationTime > m_MaxLatency) m_MaxLatency = e.m_CalculationTime;
...
void AsyncEventLayer::OnImGuiRender(Nodens::TimeStep ts) {
  ...
  ImGui::Text("Engine Time: %.2f s | FPS: %.1f", m_TimePassed,
  ...
  ImGui::Separator();
  std::lock_guard<std::mutex> lock(m_DataMutex);
  if (ImPlot::BeginPlot("Galaxy Composition Analysis",
                        ImVec2(-1, 0))) {  // -1,0 fills available space
    ImPlot::SetupAxes("Distance (Light Years)", "Atmosphere Density (g/cm^3)",
                      ImPlotAxisFlags_AutoFit, ImPlotAxisFlags_AutoFit);
    if (!m_GalaxyDistances.empty()) {
      ImPlot::PlotScatter("Planets", m_GalaxyDistances.data(),
  ...
  if (ImPlot::BeginPlot("Job Latency", ImVec2(-1, -1))) {
    ImPlot::SetupAxes("Job Index", "Seconds", ImPlotAxisFlags_AutoFit,
                      ImPlotAxisFlags_AutoFit);
    if (!m_LatencyHistory.empty()) {
      ImPlot::PlotLine("Time", m_LatencyHistory.data(),
                       (int)m_LatencyHistory.size());
    }
    ImPlot::EndPlot();
  }
  ImGui::End();
